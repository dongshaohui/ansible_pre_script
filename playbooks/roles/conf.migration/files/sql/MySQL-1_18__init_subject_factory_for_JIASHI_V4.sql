INSERT INTO POLICY_CODES (TYPE, ACTIVE, CATALOG, OPERATION, CODE) VALUES ('REAL_ESTATE_SUBJECT', 1, 'JIASHI_V4', 'VALIDATE', 'ensure(_$request.config.investProfitFeeRate).is_rate().message("投资收益手续费比例必须大于等于0，小于等于1");
...
var terminationDate = Date.parse(_$request.config.expiryDate);
ensure(isNaN(terminationDate)).equals(false).message("到期日日期格式错误");

ensure(typeof(_$request.loaneeInformation.contact)).equals("string").message("地产标需要有地产商联系方式");
');